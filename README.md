...
**This is the "full repo" version of [this gist](https://gist.github.com/EliahKagan/e83322aba8687589df874943ad203e9f)**, which got quite large. Both have effectively the same content, but this is a bit easier to navigate.
...
[License](COPYING) for *this* repository (not gitoxide): [CC0-1.0](https://creativecommons.org/publicdomain/zero/1.0/deed.en)

The following is a guide to the [files in this repo](#files) as well as some [specific notes](#specific-notes).
...
- [`main-branch-run-1-downgraded-git.log`](main-branch-run-1-downgraded-git.log)\
  A run on the main branch, on Ubuntu 22.04 LTS, using git 2.34.1.

- [`main-branch-run-2.log`](main-branch-run-2.log)\
  A run on the main branch, on Ubuntu 22.04 LTS, using git 2.45.2.

- [`main-branch-macos-run.log`](main-branch-macos-run.log)\
...
- [`gitoxide-regenerate-archives-1-prep.log`](gitoxide-regenerate-archives-1-prep.log)\
  Some checks and setup for regenerating the logs, such as installing git 2.45.2 from the PPA and testing it.

- [**`gitoxide-regenerate-archives-2-regenerate.log`**](gitoxide-regenerate-archives-2-regenerate.log)\
  **Actually regenerating the archives.**

- [`gitoxide-regenerate-archives.log`](gitoxide-regenerate-archives.log)\
...
- [`post-regeneration-rerun-1.log`](post-regeneration-rerun-1.log)\
  On the feature branch, on Ubuntu 22.04 LTS, using the generated archives, with git 2.45.2.

- [`post-regeneration-rerun-2-ignore-archives.log`](post-regeneration-rerun-2-ignore-archives.log)\
  On the feature branch, on Ubuntu 22.04 LTS, with GIX_TEST_IGNORE_ARCHIVES=1, with git 2.45.2.

- [`post-regeneration-rerun-3-downgraded-git.log`](post-regeneration-rerun-3-downgraded-git.log)\
  On the feature branch, on Ubuntu 22.04 LTS, using the generated archives, with git 2.34.1.

- [`post-regeneration-rerun-4-downgraded-git-ignore-archives.log`](post-regeneration-rerun-4-downgraded-git-ignore-archives.log)\
  On the feature branch, on Ubuntu 22.04 LTS, with GIX_TEST_IGNORE_ARCHIVES=1, with git 2.34.1.

#### macOS

- [`post-regeneration-macos-run-1.log`](post-regeneration-macos-run-1.log)\
  On the feature branch, on macOS 14, using the generated archives, with git 2.39.3.

- [`post-regeneration-macos-run-2-ignore-archives.log`](post-regeneration-macos-run-2-ignore-archives.log)\
  On the feature branch, on macOS 14, with GIX_TEST_IGNORE_ARCHIVES=1, with git 2.39.3.

#### Windows

- [`post-regeneration-windows-run-1.log`](post-regeneration-windows-run-1.log)\
  On the feature branch, on Windows 10, using the generated archives, with git 2.45.1.

- [`post-regeneration-windows-run-2-ignore-archives.log`](post-regeneration-windows-run-2-ignore-archives.log)\
...
- [**`round2-add-one-macos-specific-1-regenerate.log`**](round2-add-one-macos-specific-1-regenerate.log)\
...
- [`round2-add-one-macos-specific-2-rerun-a-fail.log`](round2-add-one-macos-specific-2-rerun-a-fail.log)

That seems to have been due to random events, since three more separate test runs afterwards all passed:

- [`round2-add-one-macos-specific-3-rerun-b-pass.log`](round2-add-one-macos-specific-3-rerun-b-pass.log)
- [`round2-add-one-macos-specific-4-rerun-c-pass.log`](round2-add-one-macos-specific-4-rerun-c-pass.log)
- [`round2-add-one-macos-specific-5-rerun-d-pass.log`](round2-add-one-macos-specific-5-rerun-d-pass.log)
...
- [`round2-post-regeneration-ubuntu.log`](round2-post-regeneration-ubuntu.log)\
...
- [`round2-post-regeneration-freebsd-run-1.log`](round2-post-regeneration-freebsd-run-1.log)\
  On the fast-forwarded feature branch on FreeBSD 14.0-RELEASE, using the generated archives, with git 2.45.1.

- [`round2-post-regeneration-freebsd-run-2-ignore-archives.log`](round2-post-regeneration-freebsd-run-2-ignore-archives.log)\
...
After regenerating the archives and committing, I [did a `gix clean -xde` and ran the tests again](post-regeneration-rerun-1.log) (on the same Ubuntu 22.04 LTS system, with the same command). One archive was changed:
...
I'm not sure why. However, this is not new. That is one of the archives that is regenerated even without `GIX_TEST_IGNORE_ARCHIVES=1` even when the tests were run on the main branch, both with the [system-provided git 2.34.1](main-branch-run-1-downgraded-git.log) and [git-core PPA git 2.45.2](main-branch-run-2.log) on Ubuntu as well as the [system-provided git on macOS](main-branch-macos-run.log). All three had the same such files:
...
[Running the tests on macOS 14.5 again, but without using pre-generated archives](post-regeneration-macos-run-2-ignore-archives.log) still passes as well. As in Ubuntu, a number of archives are modified when this is done, but I think that is normal.

### Post-testing on Windows

#### Running on Windows, using generated archives

[Running the tests on Windows 10 after the regenerated archives were in place](post-regeneration-windows-run-1.log) worked, all tests passing and no tracked files modified.

#### Running on Windows, *not* using generated archives

[Running the tests on Windows 10 again, but without using pre-generated archives](post-regeneration-windows-run-2-ignore-archives.log) produced 16 failures.
